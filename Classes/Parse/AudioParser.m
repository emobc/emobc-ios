...
-(id) init {
	if (self = [super init]) {    
		parsedLevel = [[AudioLevel alloc] init];
	}	
	return self;
}
...
static NSString * const kAudioURLElementName = @"audioUrl";
...
- (void)parser:(AQXMLParser *)parser didStartElement:(NSString *)elementName namespaceURI:(NSString *)namespaceURI qualifiedName:(NSString *)qName attributes:(NSDictionary *)attributeDict{			
	...
	AudioLevelData *theItem = [[AudioLevelData alloc] init];
...
- (void)parser:(AQXMLParser *)parser didEndElement:(NSString *)elementName namespaceURI:(NSString *)namespaceURI qualifiedName:(NSString *)qName{     
	...
	}else if ([elementName isEqualToString:kAudioURLElementName]) {
		((AudioLevelData*)self.parsedItem).audioUrl = self.currentParsedCharacterData;		
	}else if ([elementName isEqualToString:kLocalElementName]) {
		((AudioLevelData*)self.parsedItem).local = [self.currentParsedCharacterData isEqualToString:@"true"];
	}
