...
@implementation ImageGalleryParser
...
@synthesize currImageItem;

-(id) init {
	if (self = [super init]) {    
		parsedLevel = [[ImageGalleryLevel alloc] init];
	}	
	return self;
}
...
- (void)parser:(AQXMLParser *)parser didStartElement:(NSString *)elementName namespaceURI:(NSString *)namespaceURI qualifiedName:(NSString *)qName attributes:(NSDictionary *)attributeDict{			
    ...
    if ([elementName isEqualToString:kDataElementName]) {
		ImageGalleryLevelData *theItem = [[ImageGalleryLevelData alloc] init];
		...
		self.currImageItem = theImageItem;
		[theImageItem release];		
...
- (void)parser:(AQXMLParser *)parser didEndElement:(NSString *)elementName namespaceURI:(NSString *)namespaceURI qualifiedName:(NSString *)qName{     
	...
	if ([elementName isEqualToString:kDataElementName]) {
		...
		self.currImageItem.image = cachedImage;
		[cachedImage release];
		...
		self.currImageItem.nextLevel = currentNextLevel;
	}else if ([elementName isEqualToString:kImageElementName]) {
		[((ImageGalleryLevelData*)self.parsedItem) addImageItem:currImageItem];
	}
