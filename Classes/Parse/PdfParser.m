...
-(id) init {
	if (self = [super init]) {    
		parsedLevel = [[PdfLevel alloc] init];
	}	
	return self;
}
...
static NSString * const kPdfURLElementName = @"pdfUrl";
...
- (void)parser:(AQXMLParser *)parser didStartElement:(NSString *)elementName namespaceURI:(NSString *)namespaceURI qualifiedName:(NSString *)qName attributes:(NSDictionary *)attributeDict{			
	...
	PdfLevelData *theItem = [[PdfLevelData alloc] init];
...
- (void)parser:(AQXMLParser *)parser didEndElement:(NSString *)elementName namespaceURI:(NSString *)namespaceURI qualifiedName:(NSString *)qName{     
	...
	}else if ([elementName isEqualToString:kPdfURLElementName]) {
		((PdfLevelData*)self.parsedItem).pdfUrl = self.currentParsedCharacterData;		
	}else if ([elementName isEqualToString:kLocalElementName]) {
		((PdfLevelData*)self.parsedItem).local = [self.currentParsedCharacterData isEqualToString:@"true"];
	}
