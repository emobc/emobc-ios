...
-(id) init {
	if (self = [super init]) {    
		parsedLevel = [[CalendarLevel alloc] init];
	}	
	return self;
}
...
static NSString * const kEventsElementName = @"events";
static NSString * const kEventElementName = @"event";
static NSString * const kEventDateElementName = @"eventDate";
...
- (void)parser:(AQXMLParser *)parser didStartElement:(NSString *)elementName namespaceURI:(NSString *)namespaceURI qualifiedName:(NSString *)qName attributes:(NSDictionary *)attributeDict{			
	if ([elementName isEqualToString:kDataElementName]) {
		CalendarLevelData *theItem = [[CalendarLevelData alloc] init];
		self.parsedItem = theItem;
		[theItem release];
	} else if ([elementName isEqualToString:kEventElementName]) {
		AppEvents* theEvent = [[AppEvents alloc] init];
		self.currentEvent = theEvent;
		[theEvent release];		
...
- (void)parser:(AQXMLParser *)parser didEndElement:(NSString *)elementName namespaceURI:(NSString *)namespaceURI qualifiedName:(NSString *)qName{     
	if ([elementName isEqualToString:kDataElementName]) {
		...
		[((CalendarLevelData*)self.parsedItem) addEvents:self.currentEvent];
	} else if ([elementName isEqualToString:kTitleElementName]) {
		self.currentEvent.titleEvent = self.currentParsedCharacterData;
	} else if ([elementName isEqualToString:kEventDateElementName]) {
		self.currentEvent.eventDate = self.currentParsedCharacterData;
	} else if ([elementName isEqualToString:kTextElementName]) {
		self.currentEvent.textEvent = self.currentParsedCharacterData;
	} else if ([elementName isEqualToString:kTimeElementName]) {
		self.currentEvent.timeEvent = self.currentParsedCharacterData;	
		...
		self.currentEvent.nextLevel = currentNextLevel;
	}
