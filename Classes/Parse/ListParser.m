...
@synthesize currListItem;


-(id) init {
	if (self = [super init]) {    
		parsedLevel = [[ListLevel alloc] init];
	}	
	return self;
}
...
static NSString * const kListElementName = @"list";
static NSString * const kListItemElementName = @"listItem";
...
static NSString * const kCellXibElementName = @"cellXib";
...
- (void)parser:(AQXMLParser *)parser didStartElement:(NSString *)elementName namespaceURI:(NSString *)namespaceURI qualifiedName:(NSString *)qName attributes:(NSDictionary *)attributeDict{			
    ...
    if ([elementName isEqualToString:kDataElementName]) {
		ListLevelData *theItem = [[ListLevelData alloc] init];
		self.parsedItem = theItem;
		[theItem release];
	} else if ([elementName isEqualToString:kListItemElementName]) {
		ListItem *theItem = [[ListItem alloc] init];
		self.currListItem = theItem;
		[theItem release];
...
- (void)parser:(AQXMLParser *)parser didEndElement:(NSString *)elementName namespaceURI:(NSString *)namespaceURI qualifiedName:(NSString *)qName{     
	...
	if ([elementName isEqualToString:kDataElementName]) {
	...
	} else if ([elementName isEqualToString:kCellXibElementName]) {
		((ListLevelData*)self.parsedItem).cellXib = self.currentParsedCharacterData;		
		...
		[((ListLevelData*)self.parsedItem) addListItem:self.currListItem];
	} else if ([elementName isEqualToString:kTextElementName]) {
		self.currListItem.text = self.currentParsedCharacterData;
	} else if ([elementName isEqualToString:kDescriptionElementName]) {
		self.currListItem.textDescr = self.currentParsedCharacterData;
		...
		self.currListItem.itemImage = cachedImage;
		[cachedImage release];
	} else if ([elementName isEqualToString:kNextLevelElementName]) {
		self.currListItem.nextLevel = self.currNextLevel;
	}
