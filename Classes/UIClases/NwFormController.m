...
@implementation NwFormController
...
static CGFloat const kFormSubmitButtonHeightiPad = 44.8;
...
-(void) loadForm{
	...
	if(data != nil || profile != nil){
		...
		if(varStyles != nil) {
			...
			NSString *imagePath = [[NSBundle mainBundle] pathForResource:@"images/cover/backgroundCover.pn" ofType:nil inDirectory:k];
		...
		for(int i=0; i < count;i++){
			NwAppField* theField;
			if(varStyles != nil){
				theField = [data.fields objectAtIndex:i];
			}else{
...
-(float) addTextViewField:(NwAppField*)field ypos:(float)ypos withValue:(NSString*) value {
    ...
    if ([eMobcViewController isIPad]) {
		if([[UIDevice currentDevice]orientation] == UIInterfaceOrientationLandscapeLeft || [[UIDevice currentDevice]orientation] == UIInterfaceOrientationLandscapeRight){
			textFieldRounded = [[UITextView alloc] initWithFrame:CGRectMake(kFormLeftMarginiPadLandscape, textFieldYPos, kFormTextViewWidthiPad, kFormTextViewHeightiPad)];	
		}else{
			textFieldRounded = [[UITextView alloc] initWithFrame:CGRectMake(kFormLeftMarginiPad, textFieldYPos, kFormTextViewWidthiPad, kFormTextViewHeightiPad)];
		}
		
        textFieldRounded.font = [UIFont systemFontOfSize:24.0];  //font size
    } else {
		if([[UIDevice currentDevice]orientation] == UIInterfaceOrientationLandscapeLeft || [[UIDevice currentDevice]orientation] == UIInterfaceOrientationLandscapeRight){
			textFieldRounded = [[UITextView alloc] initWithFrame:CGRectMake(kFormLeftMarginLandscape, textFieldYPos, kFormTextViewWidth, kFormTextViewHeight)];	
		}else{
			textFieldRounded = [[UITextView alloc] initWithFrame:CGRectMake(kFormLeftMargin, textFieldYPos, kFormTextViewWidth, kFormTextViewHeight)];
		}
	...
	[textFieldRounded.layer setBackgroundColor:[[UIColor whiteColor] CGColor]];
	[textFieldRounded.layer setBorderColor:[[UIColor grayColor] CGColor]];
	[textFieldRounded.layer setBorderWidth:1.0];
	[textFieldRounded.layer setCornerRadius:7];
	[textFieldRounded.layer setMasksToBounds:NO];
	textFieldRounded.layer.shouldRasterize = YES;
	
	[textFieldRounded.layer setShadowRadius: 3.0f];
	[textFieldRounded.layer setShadowOffset:CGSizeMake(3, 3)];
	[textFieldRounded.layer setShadowOpacity: 1.0f];
	textFieldRounded.layer.shadowColor = [[UIColor blackColor] CGColor];
	textFieldRounded.layer.shadowOffset = CGSizeMake(3.0f, 3.0f);
	textFieldRounded.layer.shadowOpacity = 0.5f;
	textFieldRounded.layer.shadowRadius = 0.5f;

	textFieldRounded.textColor = [UIColor blackColor]; //text color	
	//textFieldRounded.placeholder = @"<enter text>";  //place holder
	textFieldRounded.editable = TRUE;
...
-(float) addImageField:(NwAppField*)field ypos:(float)ypos withValue:(NSString*) value {
	...
	NSString *imagePath = [[NSBundle mainBundle] pathForResource:data.cameraImage ofType:nil inDirectory:k];
	...
	if(![data.cameraImage isEqualToString:@""] && data.cameraImage != nil){
	...
	NSString *imagePath1 = [[NSBundle mainBundle] pathForResource:data.libraryImage ofType:nil inDirectory:k];
	...
	if(![data.libraryImage isEqualToString:@""] && data.libraryImage != nil){
		width = imageSize.image.size.width;
		height = imageSize.image.size.height;
	}else{
		width = 150;
...
-(float) addButtonSubmit:(float) ypos{
	...
	NSString *imagePath = [[NSBundle mainBundle] pathForResource:data.submitImage ofType:nil inDirectory:k];
	...
	if(![data.submitImage isEqualToString:@""] && data.submitImage != nil){
		width = imageSize.image.size.width;
		height = imageSize.image.size.height;
	}else{
		width = 150;
...
-(bool) createParameterFromForm:(NSMutableDictionary*)parameters {
	...
	for(int i=0; i < count;i++){
		...
		if (field != nil) {			
			NwAppField* theField = [data.fields objectAtIndex:i];
			if (theField != nil) {				
...
-(void) saveFormData:(NSMutableDictionary*) parameters {	
	...
	if(data != nil){
		cachedFileName = [NSString stringWithFormat:@"Form_%@.data", data.dataId];
	}else{
...
-(NSInteger)pickerView:(UIPickerView *)pickerView numberOfRowsInComponent:(NSInteger)component {	
	NwAppField* theField = [data.fields objectAtIndex:pickerView.tag];
...
-(void) orientationChangedMethod{
	...
	if(loadContent == FALSE){
		...
		if(![theAppData.topMenu isEqualToString:@""]){
			[self callTopMenu];
		}
		if(![theAppData.bottomMenu isEqualToString:@""]){
			[self callBottomMenu];
		}
