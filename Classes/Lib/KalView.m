...
static const CGFloat kHeaderHeightLandscape = 30.f;
    ...
    self.autoresizingMask = UIViewAutoresizingFlexibleHeight;
	  ...
	  if([eMobcViewController isIPad]){
	  	  if([[UIDevice currentDevice]orientation] == UIInterfaceOrientationLandscapeLeft || [[UIDevice currentDevice]orientation] == UIInterfaceOrientationLandscapeRight){
			  headerView = [[[UIView alloc] initWithFrame:CGRectMake(0.f, 0.f, 560, kHeaderHeight)] autorelease];
		  }else{
	  ...
	  if([eMobcViewController isIPad]){
		  if([[UIDevice currentDevice]orientation] == UIInterfaceOrientationLandscapeLeft || [[UIDevice currentDevice]orientation] == UIInterfaceOrientationLandscapeRight){
			  contentView = [[[UIView alloc] initWithFrame:CGRectMake(0.f, kHeaderHeight, 560, frame.size.height - kHeaderHeight)] autorelease];
		  }else{
...
- (void)addSubviewsToHeaderView:(UIView *)headerView
{
  ...
  [headerView addSubview:headerTitleLabel];
	...
	if([eMobcViewController isIPad]){
		if([[UIDevice currentDevice]orientation] == UIInterfaceOrientationLandscapeLeft || [[UIDevice currentDevice]orientation] == UIInterfaceOrientationLandscapeRight){
			nextMonthButtonFrame = CGRectMake(560 - kChangeMonthButtonWidth,
													kHeaderVerticalAdjust,
													kChangeMonthButtonWidth,
													kChangeMonthButtonHeight);
		}else{
  ...
  NSUInteger i = firstWeekday - 1;
	...
	if([eMobcViewController isIPad]){
		if([[UIDevice currentDevice]orientation] == UIInterfaceOrientationLandscapeLeft || [[UIDevice currentDevice]orientation] == UIInterfaceOrientationLandscapeRight){
			for (CGFloat xOffset = 0.f; xOffset < headerView.width; xOffset += 80.f, i = (i+1)%7) {
				CGRect weekdayFrame = CGRectMake(xOffset, 30.f, 80.f, kHeaderHeight - 29.f);
